...
- generic web-browser JSON round trips, validation, application-role
  conversion, registered launch dispatch, and URL editor interaction;
...
The signed `xcodebuild test` command passes all 91 unit tests and all seventeen
...
The signed unsandboxed suite passes all 91 unit tests and all 17 UI tests. Save
...
Generic Web Browser Resource integration has passed signed local acceptance
with Brave Browser. An Application Resource was converted through **Open as Web
Browser**, multiple URLs were saved, and Brave opened those URLs in tabs as
specified.
