...
The signed `xcodebuild test` command passes all 48 unit tests and the macOS UI
test. The build emits the expected metadata-extraction warning because WorkBench
...
adapters, and a retained test window. Its first end-to-end test covers selecting
Starter Project, renaming it, observing dirty state, explicitly saving, and
observing the saved name and cleared dirty state.
...
- additional Project and Resource selection, Resource editing, and drag
  reordering;
- Save/Discard/Cancel from selection, reload, window-close, and quit triggers;
